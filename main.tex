...
In the context of structured light, \acp{NN} have been used to classify \ac{OAM} states of classical light for long distance free-space communication, even in the presence of environmental turbulence~\cite{krenn_2014,krenn_2016,Doster_17, Park_18, Lohani_turbo_18, Li_18}\add{, and to classify structured light with fractional topological charge~{\cite{Zhanwei_oamCNN}}}.
...
Our work makes significant steps forward  with respect to previous endeavours: while Refs.~\cite{krenn_2014,krenn_2016,Doster_17, Park_18, Lohani_turbo_18, Li_18, Zhanwei_oamCNN} leverage NNs to process OAM states, our work is the first to tackle VVBs. Moreover, owing to the variety of techniques we deploy, we can address both classification and regression tasks, thus enabling the reconstruction of the input states in relevant cases of structured light beams.
...
\begin{figure}[b]
    ...
    \caption{
    {\bf a,} %\replace{Working principles of CNNs. The training set consists of a series of simulated images, plus some others collected from the experimental platform.}
\add{Schematic representation of VVBs classification via CNNs.}
    {\bf b,} 
  ...
  %  \add{The green and the red boxes highlight pictorially right and wrong classifications.}}
...
\begin{figure}
    ...
    \textbf{b,} Scaling of the average accuracy \add{$\mathcal{A}$} when classifying states into one of the $15$ VVB classes, against the fraction of experimental images added to the training set. The leftmost point refers to the case in which only simulated images are used to train the network.
    Inset: truth table\add{ reporting how the network classifies images belonging to each class}.
...
(cf. \cite{SI} and \add{Fig.~\ref{fig:class_techniques}}).
...
\add{The term \textit{accuracy} is used here to refer to the fraction of correctly classified images.}
...
\add{The addition of a small fraction of experimental images to the training set improves the capability of the network to take into account deviations of the experimental states from ideal LG modes \cite{karimi_07,karimi_09, 
...
\add{To further highlight the performance of the network, %generalization capabilities of CNNs, 
...
%\replace{To achieve this, the network should}
\add{The CNN is thus trained to} discriminate both rotations in the polarization patterns (corresponding to changes of $\phi$), and variations in the color tone (corresponding to changes of $\theta$).
%\remove{A drawback of using any type of classifier for this task is that VVBs with $|m_i|\gg1$ display a polarization pattern whose periodicity decreases as $\frac{2\pi}{|m_1-m_2|}$ (cf. Fig.~\mbox{\ref{fig:resultsCNN}}{\bf a})~\mbox{\cite{fickler2012quantum,dambrosio_gear2013}}.This complicates the recognition of $\phi$, as changes in the phase generate only small rotations of the pattern.}
% Furthermore, the classification of the VVB on the sphere requires a discretization of  $\theta$ and $\phi$, which results in dividing the sphere in sectors
To frame this as a classification task, we partition the sphere in $26$ disjoint sectors.
% \remove{The latter is divided in 26 sectors as follows}
...
% \replace{The remaining two classes, representing the two poles, correspond to $\theta \in \left[0, \frac{\pi}{8}\right]$ and $ \left[ \frac{7}{8} \pi, \pi\right]$, with no distinction in $\phi$ (cf. Fig. \mbox{\ref{fig:PCAresults}}{\bf a}).}{%
...
% \replace{The network is then trained with simulated images corresponding to different positions on the sphere. We generate $500$}
...
%\replace{This result, which is inferior with respect to what is achieved by only inferring $m_{1,2}$, confirms the suboptimality of the method when we had to artificially coarse-grain two continuous parameters.}{
%, but nonetheless provides information about the suitability of CNNs for this type of task
\add{The sub-optimality of this result is likely a consequence of framing the problem as a classification task. Indeed, partitioning makes VVBs close to the border of two sectors naturally hard to classify. Training a CNN for the corresponding regression task will potentially improve performance.}
...
\begin{figure}[t]
    ...
    \add{In the inset, we show how}, the 
    projection of each image onto its first three principal axes maps the data (orange points) onto a sphere. %\replace{The inner (outer) black (semi-transparent) sphere is added for contrast [radius equal to that of the point with smaller (larger) radius]}
    \add{To add contrast and give a better idea of how close to the surface the points are, we added a %pair of spheres (the 
    ...
    \textbf{d,}
    Average prediction accuracy \add{$\mathcal{A}$} of a linear SVM classifier, trained and tested after applying linear DR to the data, against the number of reduced dimensions $n_c$.
    For each of the 15 classes (cf. Fig.~\ref{fig:resultsCNN}a) in which the experimental dataset was divided, we show in the inset the \add{truth} table. 
    }%
...
Indeed, applying PCA to the experimental dataset of Fig.~\ref{fig:PCAresults}{\bf b}, reveals that three directions are sufficient to capture most of the information content of the images. Projecting the images along the first three principal components, we find the arranged on a three-dimensional sphere embedded in the experimental high-dimensional space (cf. inset of Fig.~\ref{fig:PCAresults}{\bf c}) \add{that allows to retrieve the state's position on the Poincar\'e sphere overcoming the border problem characterizing the previous classification method}.
...
\add{Interpreting this reduced three-dimensional representation as a Bloch sphere, we can use PCA to retrieve a complete description of the state generating a given experimental image.
% that allows to retrieve the state's position on the Poincar\'e sphere overcoming the border problem characterizing the previous classification method.
}
...
\add{The fidelity between two states is here defined in the usual way as $\mathcal F(\rho,\sigma)\equiv \Tr|\sqrt\rho\sqrt\sigma|$.}
% \add{The fidelity allows to measure the "closeness" of the two quantum states computing their inner product. }
...
%\textit{Note--} During the reviewing process of this manuscript, the authors became aware of a related work~\cite{Zhanwei_oamCNN} addressing a similar topic. %\add{Unlike our manuscript, whose purpose requires the ability to capture information stored in both the orbital angular momentum (encoded into the integer quantum numbers
% $m$) and the polarization pattern
% of a vectorial field, in Ref.~\cite{Zhanwei_oamCNN} a convolutional neural networks is trained to classify vortex beams with fractional orbital number $m$.}
...
%%  {et~al.}(2004{\natexlab{a}})\citenamefon%%t {Padgett}, \citenamefont
...
%%\bibitem [{\citenamefont {Padgett}\ \emph
%%  {et~al.}(2004{\natexlab{b}})\citenamefon%%t {Padgett}, \citenamefont
...
%%  {et~al.}(2019{\natexlab{a}})\citenamefon%%t {Cozzolino}, \citenamefont
...
%%  {http://arxiv.org/abs/https://onlinelibr%%ary.wiley.com/doi/pdf/10.1002/qute.201900038%%}
...
%%\bibitem [{\citenamefont {Cozzolino}\ %%\emph
%%  {et~al.}(2019{\natexlab{b}})\citenamefon%%t {Cozzolino}, \citenamefont
...
%%  {https://www.nature.com/articles/ncomms3%%432} {\bibfield  {journal} {\bibinfo
...
%%\bibitem [{\citenamefont {Cozzolino}\ %%\emph
%%  {et~al.}(2019{\natexlab{c}})\citenamefon%%t {Cozzolino}, \citenamefont {Bacco},
...
%%  {https://journals.aps.org/pra/abstract/1%%0.1103/PhysRevA.81.052322} {\bibfield
%%   {journal} {\bibinfo  {journal} {Phys. %%Rev. A}\ }\textbf {\bibinfo {volume}
...
%%  {https://www.nature.com/articles/ncomms1%%5516} {\bibfield  {journal} {\bibinfo
...
%%  {https://doi.org/10.1021/acsphotonics.6b%%00392} {\bibfield  {journal}
...
%%  {\bibfnamefont {N.}~\bibnamefont %%{Wiebe}}, \ and\ \bibinfo {author}
%%  {\bibfnamefont {F.}~\bibnamefont %%{Sciarrino}},\ }\href
%%  {https://journals.aps.org/prapplied/abst%%ract/10.1103/PhysRevApplied.10.044033}
