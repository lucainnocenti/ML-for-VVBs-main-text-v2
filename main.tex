...
In the context of structured light, \acp{NN} have been used to classify \ac{OAM} states of classical light for long distance free-space communication, even in the presence of environmental turbulence~\cite{krenn_2014,krenn_2016,Doster_17, Park_18, Lohani_turbo_18, Li_18}\add{, and to classify OAM states with fractional topological charge~{\cite{Zhanwei_oamCNN}}}.
...
\begin{figure}[b]
    ...
    \caption{
    {\bf a,} 
    \replace{Working principles of CNNs. The training set consists of a series of simulated images, plus some others collected from the experimental platform.}{Schematic representation of VVBs classification via CNNs.}
    {\bf b,} 
    ...
    \add{The green the red boxes highlight pictorially right and wrong classifications.}}
...
\add{The addition of a small fraction of experimental images to the training set improves the capability of the network to take into account deviations between experimental states and ideal LG modes %\cite{karimi_07,karimi_09, 
...
\add{To further highlight the generalization capabilities of CNNs, we also trained a CNN using exclusively experimental images, but using a small number of images images in the training phase. Using only $20$ images per class, we already get an accuracy of $0.99$ to classify the rest of the experimental images (which are $1668$ in total).}
...
\replace{To achieve this, the network should}{The CNN is thus trained to} discriminate both rotations in the polarization patterns (corresponding to changes of $\phi$), and variations in the color tone (corresponding to changes of $\theta$).
\remove{A drawback of using any type of classifier for this task is that VVBs with $|m_i|\gg1$ display a polarization pattern whose periodicity decreases as $\frac{2\pi}{|m_1-m_2|}$ (cf. Fig.~\mbox{\ref{fig:resultsCNN}}{\bf a})~\mbox{\cite{fickler2012quantum,dambrosio_gear2013}}.
...
\add{Partitioning makes VVBs close to the border of two sectors naturally hard to classify, but nonetheless provides information about the suitability of CNNs for this type of task.}
...
\replace{This result, which is inferior with respect to what is achieved by only inferring $m_{1,2}$, confirms the suboptimality of the method when we had to artificially coarse-grain two continuous parameters.}{The suboptimality of this result is likely consequence of framing the problem as a classification task. Training a CNN for the corresponding regression task will potentially perform much better.}
...
\begin{figure}[t]
    ...
    \add{In the inset, we show how}
    projecting each image onto its first three principal axes \remove{and rescaling} maps the data (orange points) onto a sphere\remove{, as shown in the inset}. \replace{The inner (outer) black (semi-transparent) sphere is added for contrast [radius equal to that of the point with smaller (larger) radius]}{To add contrast and give a better idea of how close to the surface the points are, we added a pair of spheres (the inner black one, and the outer semi-transparent one) with radii equal to that of the point closer and further away from the origin}.
    \textbf{d,}
    ...
    For each of the 15 classes (cf. Fig.~\ref{fig:resultsCNN}a) in which the experimental dataset was divided, we show in the inset the \replace{true-}{truth} table. 
    }%
...
\add{The fidelity is here defined in the usual way as }
...
became aware of a related work~\cite{Zhanwei_oamCNN} addressing a similar topic. \add{Unlike our manuscript, whose purpose requires the ability to capture information stored in both the orbital angular momentum (encoded into the integer quantum numbers
